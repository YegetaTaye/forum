...
//question thing
try {
  const url = `http://localhost:8000/public/api/questions/${question_id}`;
  const options = {
    method: "GET",
    headers: {
  ...
  questionDescription.innerHTML = data.question_description;
} catch (err) {
  if (err) {
    console.log(err);
  }
...
//Comment (Answers)
try {
  const url = `http://localhost:8000/public/api/answer/${question_id}`;
  const options = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
...
submit.addEventListener("click", async (event) => {
  ...
  try {
    const url = "http://localhost:8000/public/api/answers";
    const options = {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
